...
namespace UnrealFPS
{
    ...
    public class WeaponAnimationSystem : MonoBehaviour
    {
        ...
        {
            //Walk
            if (UInput.GetAxis("Horizontal") != 0 || UInput.GetAxis("Vertical") != 0)
            {
                if (UInput.GetButton("Run") && UInput.GetAxis("Horizontal") == 0 && UInput.GetAxis("Vertical") > 0)
                {
            ...
            else if (!UInput.GetButton("Fire") && !UInput.GetButton("Jump") && !stateValue[6])
            {
            ...
            //Fire
            if (UInput.GetButtonDown("Fire") && !weaponReloadSystem.BulletsIsEmpty)
            {
                stateValue[3] = true;
                animator.SetInteger("Fire", 1);
            }
            else if (UInput.GetButton("Fire") && !weaponReloadSystem.BulletsIsEmpty)
            {
                stateValue[3] = true;
                animator.SetInteger("Fire", 2);
            }
            else if (UInput.GetButtonDown("Fire") && weaponReloadSystem.BulletsIsEmpty)
            {
                stateValue[3] = true;
            ...
            {
                if (UInput.GetButtonDown("Reload") && (weaponReloadSystem.BulletCount == 0))
                {
                    stateValue[5] = true;
                    animator.SetInteger("Reload", 0);
                }
                else if (UInput.GetButtonDown("Reload") && ((weaponReloadSystem.BulletCount > 0) && (weaponReloadSystem.BulletCount < weaponReloadSystem.MaxBulletCount)))
                {
                    stateValue[5] = true;
            ...
            if (UInput.GetButtonDown("Crouch"))
            {
                stateValue[9] = true;
            }
            else if (UInput.GetButtonUp("Crouch"))
            {
                stateValue[9] = false;
            }
        ...
        {
            float factorX = -UInput.GetAxis("Mouse X") * amount;
            float factorY = -UInput.GetAxis("Mouse Y") * amount;
            ...
            float tiltAroundZ = UInput.GetAxis("Mouse X") * tiltAngle;
            float tiltAroundX = UInput.GetAxis("Mouse Y") * tiltAngle;
        ...
        public float MoveAmount
        {
            get
            {
                return Mathf.Clamp01(Mathf.Abs(UInput.GetAxis("Vertical")) + Mathf.Abs(UInput.GetAxis("Horizontal")));
            }
        }
